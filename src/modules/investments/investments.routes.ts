...
const AdviceQuerySchema = z.object({
  ...
  provider: z.enum(['anthropic', 'google', 'groq', 'xai', 'openai']).default('groq')
    .describe('Modèle IA : groq (LLaMA gratuit) | google (Gemini) | anthropic (Claude) | openai (GPT-4o) | xai (Grok)'),
})
...
const InvestmentOptionData = z.object({
  ...
  recommended: z.boolean().describe('true si accessible avec le montant disponible'),
  personalizedReason: z.string().describe('Pourquoi cette option pour cet utilisateur spécifiquement'),
})
...
const InvestmentAdviceResponse = wrapResponse(
  z.object({
    ...
    analysis: z.string().describe('Analyse personnalisée de la situation financière'),
    advice:   z.array(AdviceItemData).describe('Conseils ciblés basés sur les données réelles'),
    investmentOptions: z.array(InvestmentOptionData),
    warnings: z.array(z.string()).describe('Alertes sur des patterns préoccupants'),
  })
)
...
Obtenir des clés gratuites : [Groq](https://console.groq.com) · [Google AI](https://aistudio.google.com)`,
    ...
    handler: async (request, reply) => {
      ...
      return reply.send(ok(
        await service.getPersonalizedAdvice(request.user.sub, amount, currency, provider as LLMProvider)
      ))
    },
  })
}
